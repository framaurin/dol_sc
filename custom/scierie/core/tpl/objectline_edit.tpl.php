 ...
 * Copyright (C) 2018		François Maurin		<framauri@gmail.com>
 *
...
<!-- BEGIN PHP TEMPLATE SCIERIE objectline_edit.tpl.php -->
		...
		if (count($extrafieldsline->attribute_label) > 0)
		{
			...
			foreach($extrafieldsline->attribute_label as $key=>$label)
			{
				if (isset($_POST["options_" . $key])) {
					...
					$value = $line->array_options["options_" . $key];
				}
					
				if ($extrafieldsline->attribute_type[$key] == 'separate')
				{
					$out .= $extrafieldsline->showSeparator($key);
				}
				else
				{
					$class=(!empty($extrafieldsline->attribute_hidden[$key]) ? 'class="hideobject" ' : '');
					$csstyle=$bc[$var];
					
					// Convert date into timestamp format (value in memory must be a timestamp)
					if (in_array($extrafieldsline->attribute_type[$key],array('date','datetime')))
					{
					...
					if (in_array($extrafieldsline->attribute_type[$key],array('price','double')))
					{
						$value = isset($_POST["options_".$key])?price2num($_POST["options_".$key]):$line->array_options['options_'.$key];
					}
